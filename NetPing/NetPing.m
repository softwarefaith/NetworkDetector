...
/*
 * PingDelegate: ping发送封包后成功接收到PingResponse数据
 */
...
#pragma unused(pinger)
    assert(pinger == self.pinger);
#pragma unused(packet)
    NSString *icmpReplyType = [NSString stringWithFormat:@"%@", [SimplePing icmpInPacket:packet]->type == 129 ? @"ICMPv6TypeEchoReply" : @"ICMPv4TypeEchoReply"];
    NSString *successLog = [NSString
        stringWithFormat:@"%lu bytes from %@ icmp_seq=#%u type=%@ time=%ldms",
                         ...
                         sequenceNumber,
                         icmpReplyType,
                         [NetTimer computeDurationSince:_startTime] / 1000];
    //记录ping成功的数据
    if (self.delegate && [self.delegate respondsToSelector:@selector(appendPingLog:)]) {
